...
Rich-based CLI interface components for Project Alpha.
Provides progress bars, formatted tables, panels, and styling.
"""
...
def create_results_table(title: str, columns: List[str]) -> Optional[Table]:
...
def create_stock_table(title: str, stocks: List[Dict[str, Any]]) -> Optional[Table]:
    ...
    for stock in stocks:
        ...
        # Color signal
        if signal.upper() in ["BUY", "BULLISH", "STRONG UP", "STRONG_BUY"]:
            signal_str = f"[green bold]{signal}[/green bold]"
        elif signal.upper() in ["SELL", "BEARISH", "STRONG DOWN", "STRONG_SELL"]:
...
def print_summary_panel(
    ...
    if not RICH_AVAILABLE:
        ...
        for screener, count in results.items():
            print(f"  {screener}: {count} matches")
        return
