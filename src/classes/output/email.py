    ...
    def send_stock_report_email(
        ...
        for recipient in recipients:
            try:
                ...
                # Handle Message Size Exceeded (552)
                if e.smtp_code == 552 or 'size exceeded' in str(e).lower():
                    logger.warning(f"Email too large for {recipient} (552), attempting to send fallback CSV-only version.")
                    try:
                        # Construct lightweight message (Text + CSV only)
                        fallback_msg = MIMEMultipart()
                        fallback_msg["Subject"] = f"{subject} (Summary Only)"
                        fallback_msg["From"] = self.config.sender_id
                        fallback_msg["To"] = recipient
                        
                        fallback_body = f"""
                        <html><body>
                        <h3>Report Delivery Notice</h3>
                        <p>The full report (with images and PDF) was too large for your email provider (Max Size Exceeded).</p>
                        <p><b>Attached is the summary CSV file with all data.</b></p>
                        <p style="font-size: 12px; color: #999;">Generated by Project Alpha Automation</p>
                        </body></html>
                        """
                        fallback_msg.attach(MIMEText(fallback_body, "html"))
                        ...
                        if summary_data:
                            ...
                            attachment["Content-Disposition"] = f'attachment; filename="{filename}"'
                            fallback_msg.attach(attachment)

                        # Re-connect if needed (SMTPDataError shouldn't close connection but safe to check)
                        self._connect().sendmail(
                            fallback_msg["From"],
                            fallback_msg["To"],
                            fallback_msg.as_string().encode("utf-8"),
                        )
                        logger.info(f"Fallback (CSV-only) email sent to {recipient}")
                    except Exception as e2:
                        logger.error(f"Critical: Fallback email also failed to {recipient}: {e2}")
                else:
                    logger.error(f"SMTP Data Error sending report to {recipient}: {e}")
