...
from mplchart.primitives import OHLC, Candlesticks, Volume, TradeMarker
...
def plot_stock_indicators_svg(symbol, data, max_bars = 180):
    '''
    ...
    symbol: str
        Stock name
    data: Dataframe
        Stock data for analysis duration
    '''
    try:
        style = "tableau"
        indicators = [
        ...
        chart = Chart(title=symbol, max_bars=max_bars, style=style)
        chart.plot(data, indicators)
        #chart.figure.savefig(f"plots/{symbol}_plot.png")
        xml_data = chart.render('svg')
        # Get the current date in YYMMDD format
        timestamp = datetime.datetime.now().strftime("%y%m%d")

        # Specify the base file name and path
        base_file_name = f'Stock_plots_{timestamp}.svg'
        file_path = 'plots/' + base_file_name

        # Open the file in append mode and append the SVG data
        with open(file_path, 'ab') as svg_file:
            svg_file.write(xml_data)

        # Close the figure to release its resources
        plt.close()
    except:
        pass

def plot_stock_indicators(symbol, data, max_bars = 200):
    '''
    Plot stock data with indicators.
    ...
    try:
        ...
        chart = Chart(title=symbol, max_bars=max_bars, style=style, figsize=(18, 7))
        chart.plot(data, indicators)
        chart.figure.savefig(f"plots/{symbol}.png")
...
# Increase the figure closing threshold
matplotlib.rcParams["figure.max_open_warning"] = 700


def plot_stock_chart(symbol, data, max_bars=200):
    try:
        style = "tableau"
        indicators = [
            Candlesticks(),
